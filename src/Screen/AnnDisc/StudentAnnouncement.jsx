    ...
    <div>
      ...
      <div className='AnnBack' style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginRight: '40%' }}>
          <div>
            <h1>Announcement</h1>
            {/* Uncomment this if you want to add an announce button in the future */}
      ...
      <div>
        {getAnnouncementValue.length > 0 ? (
          getAnnouncementValue.map((item, index) => (
            <div key={item._id} style={{ display: "flex", marginBottom: 10, marginTop: 30, backgroundColor: "lightblue", justifyContent: "space-around" }}>
              <p>{item.announcement}</p>
              <p>{item.Date}</p>
            </div>
          ))
        ) : (
          <p style={{ textAlign: "center" }}>No Announcement</p>
        )}
      </div>
    </div>
  );
